...
<nav class="admin-tabs">
    ...
    >
        ...
        <?php if ($tab === 'orders'): ?>
            <div class="account-card mb-3">
                <h2 class="account-card-title">Đơn hàng</h2>

                <form method="GET" class="row g-2 mb-3">
                    <input type="hidden" name="tab" value="orders">
                    <div class="col-md-4">
                        <label class="account-label" for="status">Lọc theo trạng thái</label>
                        <select id="status" name="status" class="account-input w-100">
                            ...
                            foreach ($orderStatuses as $code => $label): ?>
                                <option
                                    ...
                                    <?php echo ($currentFilter === $code) ? 'selected' : ''; ?>
                                >
                                    <?php echo h($label); ?>
                                </option>
                            <?php endforeach; ?>
                    ...
                    <div class="table-responsive">
                        <table class="table align-middle mb-0">
                            ...
                            <?php foreach ($orders as $o): ?>
                                <tr>
                                    <td><?php echo h($o['OrderID']); ?></td>
                                    <td><?php echo h($o['FullName']); ?></td>
                                    <td><?php echo h($o['Email']); ?></td>
                                    <td><?php echo number_format($o['TotalAmount'], 0, ',', '.'); ?> đ</td>
                                    <td><?php echo h($orderStatuses[$o['Status']] ?? $o['Status']); ?></td>
                                    <td><?php echo h($o['CreatedDate']); ?></td>
                                    <td class="text-end">
                                        <form method="POST" class="d-inline-flex gap-1">
                                            ...
                                            >
                                                <?php foreach ($orderStatuses as $code => $label): ?>
                                                    <option
                                                        value="<?php echo h($code); ?>"
                                                        <?php echo ($o['Status'] === $code) ? 'selected' : ''; ?>
                                                    >
                                                        <?php echo h($label); ?>
                                                    </option>
                                                <?php endforeach; ?>
